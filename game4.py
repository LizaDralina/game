...
class Ball():
    def __init__(self, canvas, platform, ball_image_path="ball11.png"):
        self.canvas = canvas
    ...
    def draw(self, frame):
        ...
        if self.touch_platform([self.position[0], self.position[1], self.position[0] + self.radius * 2,
        ...
        y1, y2 = self.position[1], self.position[1] + self.radius * 2
        x1, x2 = self.position[0], self.position[0] + self.radius * 2

        roi = frame[y1:y2, x1:x2]
        if roi.shape[0] == self.ball_image.shape[0] and roi.shape[1] == self.ball_image.shape[1]:
            if self.ball_image.shape[2] == 4:
                alpha_mask = self.ball_image[:, :, 3] / 255.0
                for c in range(0, 3):
                    roi[:, :, c] = (1.0 - alpha_mask) * roi[:, :, c] + alpha_mask * self.ball_image[:, :, c]
            else:
                roi[:, :] = self.ball_image
...
while True:
    ...
    cv2.rectangle(frame, (platform.x, platform.y), (platform.x + platform.width, platform.y + platform.height),
                  (0, 255, 0), -1)
    ...
    for block in blocks:
        if block.check_collision(
                [ball_pos[0], ball_pos[1], ball_pos[0] + ball.radius * 2, ball_pos[1] + ball.radius * 2]):
            ball.y = -ball.y
